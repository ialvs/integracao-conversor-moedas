...
export class TransactionService {
  ...
  async convert(
    ...
    const url = `https://api.apilayer.com/exchangerates_data/convert?to=${targetCurrency}&from=${sourceCurrency}&amount=${sourceValue}`;

    const conversion: Conversion = (await JSON.parse(
      JSON.stringify(await this.fetchConversion(url, requestInit)),
    )) as Conversion;
